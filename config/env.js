  ...
  if (isProduction) {
    ...
    if (boolEnv('WEBAUTHN_ENABLED', false)) {
      if (boolEnv('WEBAUTHN_STRICT', true) === false) {
        throw new Error('Production refuses WEBAUTHN_ENABLED=true with WEBAUTHN_STRICT=false.');
      }
      if (!process.env.WEBAUTHN_RP_ID || !process.env.WEBAUTHN_ORIGIN) {
        throw new Error('WEBAUTHN_ENABLED requires WEBAUTHN_RP_ID and WEBAUTHN_ORIGIN in production.');
      }
    }

    // Mock payment must be off
    if (boolEnv('ALLOW_MOCK_PAYMENT_PROVIDER', false)) {
      throw new Error('ALLOW_MOCK_PAYMENT_PROVIDER cannot be true in production.');
    }
    if (boolEnv('ALLOW_MOCK_COMPLETE', false)) {
      throw new Error('ALLOW_MOCK_COMPLETE cannot be true in production.');
    }

    const payProvider = String(process.env.PAYMENT_PROVIDER || 'workhub_mock').toLowerCase();
    ...
    if (payProvider === 'momo') {
      if (
        ...
        throw new Error('MoMo requires MOMO_PARTNER_CODE, MOMO_ACCESS_KEY, MOMO_SECRET_KEY in production.');
      }
    }

    // Webhook secret must not fall back to JWT
    if (!process.env.GATEWAY_WEBHOOK_SECRET || process.env.GATEWAY_WEBHOOK_SECRET === process.env.JWT_SECRET) {
      throw new Error('GATEWAY_WEBHOOK_SECRET must be set and distinct from JWT_SECRET in production.');
    }

    // Public base URL
    const base = process.env.PUBLIC_BASE_URL || '';
    if (!base.startsWith('https://')) {
      throw new Error('PUBLIC_BASE_URL must be an absolute https:// URL in production.');
    }
    try {
...
const env = {
  ...
  PAYMENT_PROVIDER: String(process.env.PAYMENT_PROVIDER || (isProduction ? '' : 'workhub_mock')).toLowerCase(),
  ALLOW_MOCK_PAYMENT_PROVIDER: boolEnv('ALLOW_MOCK_PAYMENT_PROVIDER', isTest || isDev),
  ...
  WEBAUTHN_ORIGIN: process.env.WEBAUTHN_ORIGIN || process.env.PUBLIC_BASE_URL || 'http://localhost:3000',
