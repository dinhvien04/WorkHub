...
const { assertBranchAccess, resolveActingHostOwnerId } = require('../services/staffService');
...
describe('P1.9 Staff branch scope', () => {
  test('branch-limited staff cannot access other branch', async () => {
    ...
    const staff = await createUser({ email: 's-br@test.com', role: 'customer' });
...
describe('P1.6 Recurring cancel releases slots', () => {
  test('cancel whole series cancels children and frees slots', async () => {
    ...
    const result = await recurringService.cancelSeries(series._id, customer._id, {
      mode: 'whole',
    });
...
describe('P1.8 Check-in random code + no-show grace', () => {
  test('mint uses random code not ObjectId suffix; no-show needs grace', async () => {
    ...
    const customer = await createUser({ email: 'cci@test.com', role: 'customer' });
    const { space } = await seedHostSpace(host);
    const day = new Date();
...
describe('P1.13 Inline handler CI guard', () => {
  test('no new onclick in critical security.js / api.js', () => {
    const fs = require('fs');
    const path = require('path');
    for (const f of ['public/js/api.js', 'public/js/domSafe.js', 'public/js/security.js']) {
