...
describe('P0.2 Google OIDC account safety', () => {
  test('mock creates new user; collision blocked', async () => {
    ...
    await expect(googleOidc.mockLogin({ email: 'taken@ex.com' })).rejects.toMatchObject({
      statusCode: 409,
    });
  });
});
...
describe('P0.3 Email verification gate', () => {
  test('register customer inactive; login blocked; confirm activates', async () => {
    const reg = await request(app).post('/api/auth/register').field('email', 'newc@test.com').field('password', 'Pass1234ab').field('fullName', 'New C').field('role', 'customer').field('contactPhone', '0901111222');
  ...
  test('inactive unverified local user cannot login', async () => {
    ...
    expect(String(res.body.error || res.body.message || '')).toMatch(/email|kích hoạt|xác minh/i);
    ...
    const conf = await request(app).post('/api/auth/email/confirm').send({ token: raw });
...
describe('P0.4-5 Gateway amount server-side + mock complete', () => {
  test('checkout uses paymentType not client amount; overpay rejected', async () => {
    ...
    const customer = await createUser({ email: 'gc@test.com', role: 'customer' });
  ...
  test('mock complete route exists only when ALLOW_MOCK_COMPLETE', async () => {
    ...
    const res = await withCsrf(
      request(app).post(`/api/gateway/sessions/${session.SessionId}/mock-complete`),
      csrf,
      `authToken=${token}`
    );
...
describe('P0.9 Partial refund allocation', () => {
  test('partial refund keeps remaining successful net', async () => {
    ...
    const customer = await createUser({ email: 'rc@test.com', role: 'customer' });
    const { space } = await seedHostSpace(host);
...
describe('P1.1 Hold expiry includes pending', () => {
  test('expired pending hold is cleaned', async () => {
    ...
    const customer = await createUser({ email: 'ec@test.com', role: 'customer' });
    const { space } = await seedHostSpace(host);
    ...
    await Booking.updateOne(
      { _id: booking._id },
      { $set: { Status: 'pending', HoldExpiresAt: new Date(Date.now() - 60000) } }
    );
