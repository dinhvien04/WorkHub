...
const p = path.join(
  __dirname,
  "..",
  "WorkHub_Master_Features_UX_SEO_Performance_Security_Prompt.md",
);
...
const partialNotes = {
  ...
  "Availability calendar realtime, timezone rõ, không chọn quá khứ/ngoài giờ.":
    null,
  "Instant booking và booking request là hai luồng rõ ràng.": null,
  ...
  "Favorites và collection.": " *(favorites + merge; collection/folder chưa)*",
  "Support ticket/chat theo booking, evidence, SLA.":
    " *(ticket/chat; evidence/SLA mỏng)*",
  "Không dùng Tailwind CDN production; build, purge, minify, hash CSS.":
    " *(purge/minify; hash filename 1y immutable chưa)*",
  "Không trả nguyên Mongoose document; dùng DTO/presenter.": null, // partial presenters - leave open or tick partial
};
...
for (const [line, note] of Object.entries(partialNotes)) {
  ...
  if (
    s.includes(ticked) &&
    !s.includes(ticked + note) &&
    !s.includes(line + note)
  ) {
...
if (
  s.includes("- [ ] Instant booking và booking request là hai luồng rõ ràng.")
) {
  s = s.replace(
    "- [ ] Instant booking và booking request là hai luồng rõ ràng.",
...
if (
  s.includes("- [ ] Không trả nguyên Mongoose document; dùng DTO/presenter.")
) {
  s = s.replace(
    "- [ ] Không trả nguyên Mongoose document; dùng DTO/presenter.",
