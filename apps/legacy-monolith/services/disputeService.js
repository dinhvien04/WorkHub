...
async function resolveDispute({
  ...
  return withTransaction(async (session) => {
    ...
    if (d.RefundAmount > 0) {
      ...
      if (refund) {
        const processed = await refundService.processRefund({
          refundId: refund._id,
          actorId: adminId,
          approve: true,
          role: "admin",
        ...
        if (processed) {
          if (processed.Status === "completed") {
            d.Status = "resolved";
          } else if (["provider_pending", "provider_submitted", "manual_refund_required"].includes(processed.Status)) {
            d.Status = "refund_pending";
          } else if (processed.Status === "failed") {
            d.Status = "resolution_failed";
          }
          await d.save(session ? { session } : undefined);
        }
      }
