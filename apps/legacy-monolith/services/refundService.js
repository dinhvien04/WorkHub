...
async function processRefund({
  ...
  // After provider_pending: submit provider refunds via outbox/worker
  if (completed && completed.Status === "provider_pending" && submitProvider) {
    const outboxService = require("./outboxService");
    await outboxService.enqueue(
      {
        type: "provider_refund",
        entityType: "Refund",
        entityId: completed._id,
        payload: { refundId: completed._id },
        idempotencyKey: `provider-refund:${completed._id}`,
      },
      { session },
    );
  }
