...
reviewSchema.statics.calcAverageRatings = async function (spaceId) {
  try {
    ...
    if (spaceStats.length > 0) {
      ...
      await Space.findByIdAndUpdate(spaceId, { RatingAvg: 0 });
    }
  } catch (error) {
    const logger = require("../utils/logger");
    logger.error({ err: error, spaceId }, "Lỗi khi tính toán RatingAvg cho Branch");
  }
};
...
reviewSchema.post("save", async function () {
  await this.constructor.calcAverageRatings(this.SpaceID);
});
