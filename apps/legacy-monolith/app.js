...
function createApp() {
  ...
  app.post(
    ...
    require("./controllers/billingController").gatewayWebhook,
  );
  app.post(
  ...
  // Attach verified identity (incl. sid) BEFORE CSRF so tokens bind to session
  app.use(require("./middlewares/authMiddleware").optionalAuth);
  ...
  app.use((req, res, next) => {
    ...
    const skip =
      ...
      (req.path === "/api/auth/webauthn/login/options" &&
        req.method === "POST") ||
      (req.path === "/api/auth/webauthn/login/verify" &&
        req.method === "POST") ||
      (req.path === "/api/auth/google/mock" && req.method === "POST") ||
      (req.path === "/api/auth/csrf" && req.method === "GET") ||
      (req.path === "/api/i18n/lang" &&
  ...
  app.use((req, res, next) => {
    ...
    res.locals.useTailwindCdn =
      process.env.USE_TAILWIND_CDN === "1" ||
      process.env.USE_TAILWIND_CDN === "true"
        ? true
        : process.env.USE_TAILWIND_CDN === "0" ||
            process.env.USE_TAILWIND_CDN === "false"
          ? false
  ...
  app.get("/login", (req, res) =>
    res.render("customer/login", { pageTitle: "Đăng nhập — WorkHub" }),
  );
  app.get("/register", (req, res) =>
    res.render("customer/register", { pageTitle: "Đăng ký — WorkHub" }),
  );
  ...
  app.get("/hosts/:hostId", async (req, res, next) => {
    try {
      ...
      if (e.statusCode === 404)
        return res.status(404).send("Không tìm thấy host");
      return next(e);
    }
  });
