...
describe('Review status filtering and privacy', () => {
  ...
  test('last review hidden -> rating to 0', async () => {
    const host = await createUser({ email: 'h_last_hidden@test.com', role: 'host' });
    const customer = await createUser({ email: 'c_last_hidden@test.com', role: 'customer' });
    const { space, branch } = await seedHostSpace(host);
    ...
    // Create single published review
    const review = await Review.create({
      SpaceID: space._id,
      CustomerID: customer._id,
      BookingID: booking._id,
    ...
    // Verify rating average starts at 5.0
    const SpaceModel = require('../models/Space');
    const BranchModel = require('../models/Branch');
    let updatedSpace = await SpaceModel.findById(space._id);
    let updatedBranch = await BranchModel.findById(branch._id);
    expect(updatedSpace.RatingAvg).toBe(5);
    expect(updatedBranch.RatingAvg).toBe(5);
    ...
    // Verify rating average recalculates to 0
    updatedSpace = await SpaceModel.findById(space._id);
    updatedBranch = await BranchModel.findById(branch._id);
    expect(updatedSpace.RatingAvg).toBe(0);
    expect(updatedBranch.RatingAvg).toBe(0);
  });
});
