...
describe("Push subscription security, limit, and rate limiters", () => {
  ...
  test("prevents SSRF by blocking private/loopback/link-local/multicast IPs", async () => {
    ...
    for (const ep of endpoints) {
      ...
      expect(res.body.error).toMatch(/SSRF|hợp lệ|tin cậy|được phép/i);
    }
  });

  test("prevents DNS Rebinding by rejecting hostnames not in allowlist", async () => {
    const user = await createUser({ email: "p-dns@test.com", role: "customer" });
    const { token } = agentWithAuth(app, user);
    const csrf = await getCsrfPair(app);
    ...
    const res = await withCsrf(
      request(app)
        .post("/api/push/subscribe")
        .send({ endpoint: "https://attacker.com/push" }),
      csrf,
      `authToken=${token}`,
    );

    expect(res.status).toBe(400);
    expect(res.body.error).toMatch(/tin cậy|được phép/i);
  });
