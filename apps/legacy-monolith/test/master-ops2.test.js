...
describe('Hold reminder job', () => {
  ...
  test('PATCH /api/host/spaces/:spaceId/ops freeCancelHours = 0', async () => {
    const host = await createUser({ email: 'hops@test.com', role: 'host' });
    const { space } = await seedHostSpace(host);
    ...
    const res = await withCsrf(
      request(app)
        .patch(`/api/host/spaces/${space._id}/ops`)
        .send({ freeCancelHours: 0 }),
      csrf,
      `authToken=${token}`
    );

    expect(res.status).toBe(200);
    expect(res.body.space.FreeCancelHours).toBe(0);

    const Space = require('../models/Space');
    const updatedSpace = await Space.findById(space._id);
    expect(updatedSpace.FreeCancelHours).toBe(0);
  });
});
