...
function seoPathForBranch(branch) {
  if (!branch || !branch.Slug)
    return `/detail?branchId=${branch && branch._id}`;
 ...
 */
async function buildDetailViewModel(
  branch,
  { req, spaces: preloadedSpaces } = {},
) {
  ...
  const desc =
    ...
    `${branch.Name} · ${branch.Address || ""} · Đặt chỗ co-working trên WorkHub`.slice(
      0,
      160,
    );
  ...
  const localBusiness = {
    ...
    priceRange: minPrice
      ? `Từ ${minPrice.toLocaleString("vi-VN")}đ/giờ`
      : undefined,
    geo:
  ...
  branch.RatingAvg = breakdown.total
    ? breakdown.average
    : branch.RatingAvg || 0;
  ...
  for (const s of spaces) {
    if (Array.isArray(s.Amenities))
      s.Amenities.forEach((a) => a && amenitySet.add(String(a)));
  ...
  const policies = {
    ...
    depositPercent:
      branch.DepositPercentage != null
        ? Math.round(Number(branch.DepositPercentage) * 100)
        : 30,
