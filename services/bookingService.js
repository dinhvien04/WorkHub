...
function assertTransition(from, to) {
  ...
  if (!allowed.includes(to)) {
    throw new ValidationError(
      `Không thể chuyển trạng thái booking từ "${from}" sang "${to}".`,
    );
  }
}
...
function validateBookingWindow(start, end) {
  ...
  if (end <= start)
    throw new ValidationError("EndTime phải lớn hơn StartTime.");
...
async function createBooking({
  ...
  if (!spaceId || !startTime || !endTime) {
    throw new ValidationError(
      "Thiếu thông tin đặt chỗ (spaceId, startTime, endTime).",
    );
  }
  ...
  const holdMs =
    Math.min(Math.max(Number(holdMinutes) || 15, 5), 60) * 60 * 1000;
  ...
  return withLock(`booking:space:${spaceId}`, () =>
    withOptionalTransaction(async (session) => {
      ...
      const bufferBefore =
        Math.max(0, Number(space.BufferBeforeMinutes) || 0) * 60 * 1000;
      const cleanupAfter =
        Math.max(0, Number(space.CleanupAfterMinutes) || 0) * 60 * 1000;
    ...
    const booking = await Booking.findOneAndUpdate(
      {
        ...
        HostID: hostId,
        Status: {
          $in: ["pending", "payment_under_review", "awaiting_payment"],
        },
      },
...
async function checkInBooking(hostId, bookingId) {
  ...
  if (!booking) {
    ...
    throw new ValidationError(
      "Chỉ có thể nhận phòng với đơn đã được xác nhận.",
    );
  }
...
async function cancelBookingByCustomer(customerId, bookingId, reason = "") {
  ...
  await PaymentHistory.updateMany(
    { BookingID: bookingId, CustomerID: customerId, Status: "pending" },
    {
      $set: {
        Status: "failed",
        FailureReason: "Booking cancelled by customer",
      },
    },
  );
  await logActivity(
