...
async function postCreditEntry({
  ...
  if (idempotencyKey) {
    const existing = await MembershipCreditLedger.findOne({ IdempotencyKey: idempotencyKey });
...
async function consumeCredit(userId, hours = 1, { bookingId = null, idempotencyKey = null } = {}) {
  ...
  const entry = await postCreditEntry({
    ...
    bookingId,
    idempotencyKey: idempotencyKey || (bookingId ? `consume-${bookingId}-${hours}` : null),
 ...
 */
async function expireCredits(membershipId, { reason = 'membership_ended' } = {}) {
  const m = await Membership.findById(membershipId);
  if (!m) throw new NotFoundError('Membership không tồn tại.');
...
async function listCreditLedger(userId, { page = 1, limit = 50 } = {}) {
  ...
  const [items, total] = await Promise.all([
    MembershipCreditLedger.find(filter).sort({ createdAt: -1 }).skip(skip).limit(limit).lean(),
 ...
 */
async function grantCredits({ userId, hours, description = 'Cấp thêm credit', meta = {} }) {
  const m = await Membership.findOne({
    UserID: userId,
    Status: 'active',
    EndsAt: { $gt: new Date() },
