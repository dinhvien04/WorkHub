...
const ALL_PROVIDERS = ['workhub_mock', 'stripe_mock', 'momo_mock', 'stripe', 'momo'];
...
function activeProvider(requested) {
  const fromEnv = String(env.PAYMENT_PROVIDER || process.env.PAYMENT_PROVIDER || '').toLowerCase();
  ...
  if (!ALL_PROVIDERS.includes(p)) return mockAllowed() ? 'workhub_mock' : fromEnv || 'stripe';
  return p;
}
...
async function tryCreateLiveSession({
  ...
  if (provider === 'stripe' && stripeLiveReady()) {
    ...
    params.set(
      'success_url',
      successUrl || `${env.PUBLIC_BASE_URL || ''}/payment/gateway/{CHECKOUT_SESSION_ID}`
    );
    params.set('cancel_url', cancelUrl || `${env.PUBLIC_BASE_URL || ''}/payment`);
...
function webhookSecretFor(provider) {
  if (provider === 'stripe' || provider === 'stripe_mock') {
    return process.env.STRIPE_WEBHOOK_SECRET || process.env.GATEWAY_WEBHOOK_SECRET || '';
  }
  if (provider === 'momo' || provider === 'momo_mock') {
    return process.env.MOMO_SECRET_KEY || process.env.GATEWAY_WEBHOOK_SECRET || '';
  }
  // workhub_mock — never use JWT_SECRET as fallback in production (validated at startup)
  return process.env.GATEWAY_WEBHOOK_SECRET || (env.isProduction ? '' : env.JWT_SECRET);
}
...
function listProviders() {
  ...
  if (stripeLiveReady()) items.push({ id: 'stripe', name: 'Stripe', live: true });
  else if (mockAllowed()) items.push({ id: 'stripe_mock', name: 'Stripe (mock)', live: false });
  if (momoLiveReady()) items.push({ id: 'momo', name: 'MoMo', live: true });
  else if (mockAllowed()) items.push({ id: 'momo_mock', name: 'MoMo (mock)', live: false });
  return items;
}
...
function verifyMomoIpn(event, secret) {
  ...
  const expected = crypto.createHmac('sha256', secret).update(raw).digest('hex');
  try {
    return crypto.timingSafeEqual(Buffer.from(String(event.signature || '')), Buffer.from(expected));
  } catch {
    return false;
  }
}
