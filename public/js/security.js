...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function loadSessions() {
    ...
    items.forEach((s) => {
      ...
      card.appendChild(
        DomSafe.createTextElement('p', 'font-bold text-slate-800', s.UserAgent || s.DeviceLabel || 'Thiết bị')
      );
      const meta = [
        s.IP ? 'IP: ' + s.IP : null,
  ...
  if (deleteBtn) {
    deleteBtn.addEventListener('click', async () => {
      if (!confirm('Gửi yêu cầu xóa tài khoản? Hành động cần xác nhận admin và không hoàn tác dữ liệu tài chính.')) {
        return;
      }
      try {
